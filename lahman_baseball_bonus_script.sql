...
-- If you need a hint, you can structure your query as follows: <Write a correlated subquery here that will pull the teamid for the team with the highest number of wins from each league>

SELECT DISTINCT 
	lgid, 
	(SELECT 
		CASE	WHEN MAX(w) = w THEN teamid END AS teamid
	FROM teams)
FROM teams t
WHERE yearid = 2016
GROUP BY w;
...
SELECT * 
FROM
(SELECT DISTINCT lgid,MAX(W) AS highest_wins
FROM teams
WHERE yearid=2016
GROUP BY lgid) AS most_wins
INNER JOIN teams
ON teams.lgid=most_wins.lgid AND teams.w=most_wins.highest_wins
WHERE yearid=2016;

SELECT DISTINCT lgID, (SELECT teamid, w
						FROM teams
						WHERE yearid=2016
						GROUP BY teamid, w
						HAVING w = (SELECT MAX(w)
										FROM teams))
FROM teams
WHERE yearid = 2016;

SELECT DISTINCT lgid,
(select teamid FROM teams t1
WHERE t1.lgid=t2.lgid AND t1.yearid=2016 )AS teams_id,
(SELECT MAX(w) FROM teams t1
WHERE t1.lgid=t2.lgid AND t1.yearid=2016
ORDER BY w
)AS Highest_wins
FROM teams t2
GROUP BY w;
